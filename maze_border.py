...
class MazeGame:
    ...
    def get_move(self, keys, maze: list[list]) -> tuple[int, int, bool]:
        if keys[pygame.K_LEFT] and self.player_x > 0 and maze[self.player_y][self.player_x - 1] == 0:
            self.player_x = self.player_x - 1
            return True
        elif keys[pygame.K_RIGHT] and self.player_x < COLS - 1 and maze[self.player_y][self.player_x + 1] == 0:
            self.player_x = self.player_x + 1
            return True
        elif keys[pygame.K_UP] and self.player_y > 0 and maze[self.player_y - 1][self.player_x] == 0:
            self.player_y = self.player_y - 1
            return True
        elif keys[pygame.K_DOWN] and self.player_y < ROWS - 1 and maze[self.player_y + 1][self.player_x] == 0:
            self.player_y = self.player_y + 1
            return True
        return False
    ...
    def run(self):
        ...
        maze[self.end_y1][self.end_x1] = 0  # Place heart at (1, 1) inside the maze
        maze[self.end_y2][self.end_x2] = 0  # Place brain at (COLS-2, 1) inside the maze
        maze, trail, self.player_x, self.player_y = self.maze.reset(maze)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
            self.turn(maze, trail)
            pygame.display.update()

        pygame.quit()

    def turn(self, maze: list[list], trail) -> None:
        ...
        if current_time - self.last_move_time > MOVE_DELAY:
            ...
            moved = self.get_move(keys, maze)

            if moved:
                self.move(trail)
                self.last_move_time = current_time

            if (self.player_x, self.player_y) == (self.end_x1, self.end_y1):
            ...
            elif (self.player_x, self.player_y) == (self.end_x2, self.end_y2):
                screen.fill(DARK_BLUE)
                font = pygame.font.Font(None, 50)
        ...
        screen.blit(heart_img, ((self.end_x1 + BORDER_WIDTH) * CELL_SIZE, (self.end_y1 + BORDER_WIDTH) * CELL_SIZE))
        screen.blit(brain_img, ((self.end_x2 + BORDER_WIDTH) * CELL_SIZE, (self.end_y2 + BORDER_WIDTH) * CELL_SIZE))
        ...
        screen.blit(player_img, ((self.player_x + BORDER_WIDTH) * CELL_SIZE, (self.player_y + BORDER_WIDTH) * CELL_SIZE))
