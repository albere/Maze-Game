...
func _ready():
	...
	dpad.rect_min_size = Vector2(200, 200)  # Adjust the size as needed
	dpad.rect_position = Vector2(get_viewport().size.x - dpad.rect_min_size.x - 16, get_viewport().size.y - dpad.rect_min_size.y - 16)
